/**
 * Seed the PGA flag reference tables from a parsed ACE Agency Tariff Code
 * Reference JSON (produced by scripts/parse-ace-flags.ts).
 *
...
const DEFAULT_SEED = resolve(
  import.meta.dir,
  "../src/db/reference/pga-flags.json",
);
...
for (let start = 0; start < flags.length; start += CHUNK) {
  ...
  console.log(`  inserted ${Math.min(start + CHUNK, flags.length)}/${flags.length}`);
}

// Flip the active pointer atomically: deactivate the old versions and
// activate the new one in a single transaction.
await db.transaction(async () => {
  const { data: activeVersions } = await listPgaFlagVersions({ active: true });
