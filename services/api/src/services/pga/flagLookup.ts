 ...
 * flag-table publication attach to an HTS code? Pure lookup — the judgment
 * about whether a flagged agency actually applies to a given shipment
 * belongs to the screening agent, never to this layer.
 */
export class PgaFlagLookupService {
  /** Digits only — the reference and our table store undotted prefixes. */
  static normalizeHtsCode(htsCode: string): string {
    return htsCode.replace(/\D/g, "");
  }

  /** Even-length prefixes (2–10 digits) the reference can list a code at. */
  static expandHtsPrefixes(htsCode: string): string[] {
    const digits = PgaFlagLookupService.normalizeHtsCode(htsCode);
    return [2, 4, 6, 8, 10]
      .filter((length) => digits.length >= length)
      .map((length) => digits.slice(0, length));
  }

  static async lookup(htsCode: string): Promise<PgaFlagLookupResult> {
    ...
    // One agency+flag can match at several prefix lengths (e.g. a
    // chapter-wide AQ1 and a line-specific AQ2); keep the most specific
    // match per pair.
